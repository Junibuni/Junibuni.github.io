...
차량 손상 감지와 같은 세그멘테이션 작업은 RGB 이미지와 같은 다양한 입력 유형과 복잡한 형상을 처리해야 하므로, 신중한 아키텍처 선택이 필수적이다. Vanilla U-Net을 실험하면서 그 한계를 인식한 후, VGG19, ResNet50, EfficientNetB0와 같이 깊이 있는 모델을 사용했다.

### Why deeper backbones?
Vanilla U-Net에서 경험한 문제는 얕은 계층과 낮은 특징 맵 깊이로 인해 더욱 복잡한 특징 추출기의 필요성을 느끼게 되었다. 이에 따라 VGG19, ResNet50, EfficientNetB0 백본을 실험하게 되었으며, 해당 모델들은 깊이와 계층적 특징 추출을 통해 세밀한 디테일을 포착할 수 있는 모델로 널리 알려져 있다.

#### VGG19:
VGG19는 19개의 계층으로 구성된 심층 신경망으로, 주로 3x3 합성곱 계층을 쌓아 복잡한 작업을 처리하는 데 강력한 성능을 보이는 모델이다. 비교적 간단하지만, 이미지 분류와 같은 작업에서 뛰어난 성능을 보이는 모델이기 때문에 선택했다.

#### ResNet50:
ResNet50은 잔차 연결(Residual connections)을 도입하여 깊은 모델에서도 기울기 정보가 원활하게 흐를 수 있도록 해준다. 50개의 계층을 통해 차량의 흠집이나 찌그러짐과 같은 세밀한 손상을 포착할 수 있는 계층적 표현을 학습할 수 있디. 잔차 블록 덕분에 그라디언트 소실 문제를 완화하는 ResNet50은 이 세그멘테이션 작업에 적합한 백본 중 하나라고 생각했다.

#### EfficientNetB0:
EfficientNetB0는 복합 스케일링(compound scaling)과 깊이별 분리 합성곱(depth-wise separable convolutions)을 사용하여 모델의 복잡도와 성능을 모두 최적화 한다. 이로 인해 계산 효율성을 유지하면서도 높은 정확도를 제공하는 모델이다.

### 결과 및 분석

고급 백본을 U-Net 구조에 통합한 후, Dice, F1, 정밀도(Precision), 재현율(Recall), Jaccard 지수와 같은 지표를 기준으로 모델을 비교했습니다.
...
세 가지 모델 중 **EfficientNetB0**가 가장 뛰어난 성능을 보였으며, 속도와 정확성의 균형이 우수했습니다. 이 모델의 효율적인 아키텍처 덕분에 더 복잡한 ResNet50과 VGG19 백본보다 특히 정밀도와 재현율 측면에서 더 나은 성능을 보였습니다. 이는 차량의 미세한 손상을 정확하게 감지하는 데 중요한 요소입니다.

### 결론

VGG19, ResNet50, EfficientNetB0와 같은 고급 백본을 탐구한 결과, 차량 손상 세그멘테이션 작업에 U-Net을 최적화하는 방법에 대한 더 깊은 통찰을 얻을 수 있었습니다. 실험 결과, 고급 사전 학습 모델을 활용하는 것이 고수준 특징 추출이 중요한 작업에서 매우 유리하다는 것을 보여주었습니다. 특히 **EfficientNetB0**는 성능과 계산 비용의 균형을 잘 맞춘 백본으로서, 실시간 손상 감지 작업에 가장 적합한 모델로 입증되었습니다.

이번 실험을 통해 모델 조정의 복잡성에 대한 소중한 경험을 쌓았으며, 모델의 깊이, 성능, 계산 효율성 간의 트레이드오프를 더 잘 이해하게 되었습니다.