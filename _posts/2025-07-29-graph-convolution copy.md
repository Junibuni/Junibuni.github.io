...
- \\( H^{(l)} \\): layer \\( l \\)의 노드 feature matrix
- \\( W^{(l)} \\): 학습할 가중치 행렬
- \\( \hat{A} = A + I \\): self-loop 포함한 인접행렬
- \\( \hat{D} \\): \\( \hat{A} \\)에 대한 degree matrix
- \\( \sigma \\): 활성화 함수
...
<div style="text-align: center;">
  <img src="GCN_compare_graph_norm.png
...
GCN은 단순히 이웃의 정보를 더하는 것이 아니라, **정규화된 연결 정보에 기반해**
"얼마나" 어떤 이웃의 정보를 받아올지를 세심하게 조절하는 구조입니다.
특히 **degree 행렬을 양쪽에서 제곱근 역수로 곱하는 정규화**는
그래프 구조의 특성과 학습의 안정성 사이의 균형을 맞추는 중요한 수학적 장치입니다.
